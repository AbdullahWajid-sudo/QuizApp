...
function Answers(props) {
  const questions = [...(props.que || [])].reverse();
  const answers = [...(props.ans || [])].reverse();

  const tableData = questions.map((question, index) => {
    const uAns = answers[index] || "Not Answered";

    // Ensure we match Firestore field names
    const cAns = question.correctAnswer || question.answer || "N/A";
    const qText =
      question.questionText || question.question || "No Question Text";

    const isCorrect =
      uAns?.toString().trim().toLowerCase() ===
      cAns?.toString().trim().toLowerCase();
